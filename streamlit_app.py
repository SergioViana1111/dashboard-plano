...
if not st.session_state.logged_in:
    ...
    if uploaded_file:
        ...
        # DASHBOARD TABS
        # ---------------------------
        tabs=["📊 KPIs Gerais","📈 Comparativo","🚨 Alertas","🔍 Busca","📤 Exportação"] if role=="RH" else ["🏥 Análise Médica","🔍 Busca"]
        ...
        for i, tab_name in enumerate(tabs):
            with tab_objs[i]:
                if tab_name=="📊 KPIs Gerais":
                    ...
                    custo_total=utilizacao['Valor'].sum() if 'Valor' in utilizacao.columns else 0
                    volume_total=len(utilizacao)
                    num_beneficiarios=utilizacao['Nome_do_Associado'].nunique() if 'Nome_do_Associado' in utilizacao.columns else 0
                    ...
                    plano_col=[c for c in utilizacao.columns if 'plano' in c.lower() and 'descricao' in c.lower()]
                    if plano_col and 'Valor' in utilizacao.columns:
                        plano_col=plano_col[0]
                        comp=utilizacao.groupby(plano_col)['Valor'].sum().reset_index()
                        fig=go.Figure(data=[go.Bar(x=comp[plano_col], y=comp['Valor'], text=comp['Valor'].apply(format_brl), textposition='outside')])
                    ...
                    custo_por_benef=utilizacao.groupby('Nome_do_Associado')['Valor'].sum()
                    top10_volume=utilizacao.groupby('Nome_do_Associado').size()
                    ...
                    if search_name:
                        ...
                        df_result=utilizacao[utilizacao['Nome_do_Associado'].apply(lambda x: normalize_name(x)==norm_name)]
                        st.dataframe(style_dataframe_brl(df_result))
                
                elif tab_name=="📤 Exportação":
                    st.markdown("### 📤 Exportar Dados")
                    st.download_button("⬇️ Exportar Utilização", data=utilizacao.to_csv(index=False).encode('utf-8'), file_name='utilizacao.csv')
